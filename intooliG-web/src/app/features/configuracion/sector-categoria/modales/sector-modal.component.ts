...
import { SectorDto } from '../servicios/sector.service';
  ...
  template: `
    <div *ngIf="visible" class="dialog-backdrop">
      <div class="dialog-card">
        <h5>{{ editId ? 'Editar sector' : 'Nuevo sector' }}</h5>
        <div class="mb-2">
...
export class SectorModalComponent {
  @Input() visible = false;
  @Input() saving = false;
  @Input() set sector(v: SectorDto | null) {
    if (v) {
      this.editId = v.id;
      this.nombre = v.nombreSector;
  ...
  @Output() save = new EventEmitter<{ nombreSector: string; activo: boolean }>();
  ...
  guardar(): void {
    this.save.emit({ nombreSector: this.nombre.trim(), activo: this.activo });
  }
}
